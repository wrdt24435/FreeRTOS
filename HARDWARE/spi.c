...
{	 
	...
	SPI_Init((SPI_TypeDef *)spi_port[W25Q16_SPI], &SPI_InitStructure);  //根据SPI_InitStruct中指定的参数初始化外设SPIx寄存器

	SPI_Cmd((SPI_TypeDef *)spi_port[W25Q16_SPI], ENABLE); //使能SPI外设

	SPI_ReadWriteByte(W25Q16_SPI, 0xff);//启动传输		 
	SPI_Cmd((SPI_TypeDef *)spi_port[W25Q16_SPI],ENABLE); //使能SPI1 
} 
...
{		 			 
	...
	while (SPI_I2S_GetFlagStatus((SPI_TypeDef *)spi_port[port], SPI_I2S_FLAG_TXE) == RESET){}//等待发送区空  
	SPI_I2S_SendData((SPI_TypeDef *)spi_port[port], TxData); //通过外设SPIx发送一个byte  数据	
	while (SPI_I2S_GetFlagStatus((SPI_TypeDef *)spi_port[port], SPI_I2S_FLAG_RXNE) == RESET); //等待接收完一个byte  
	return SPI_I2S_ReceiveData((SPI_TypeDef *)spi_port[port]); //返回通过SPIx最近接收的数据	
